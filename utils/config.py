...
class ProjectConfig:
    ...
    learning_rate: float = 1e-4 # LR for Hybrid-Order Perceptual Network
    ...
    lambda_reg: float = 0.3     # PF Block regularization weight (MMD + Alignment)
    lambda_perc: float = 0.5  # Hybrid-Order Perceptual loss weight

    # Conditioning Strategies
    use_saliency: bool = True   # Enable Saliency Map from Diagnosis Guidance Network
    ...
    CHECKPOINT_G_diag: str = os.path.join(exp_root, exp_da, "G_diag_NET.pth.tar")
    CHECKPOINT_Phi: str = os.path.join(exp_root, exp_mri_pet, "Phi.pth.tar")
