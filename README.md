...
To ensure proper rendering across all platforms (including GitHub, VS Code, Typora, etc.), the directory tree is shown below in a properly indented code block. If the tree still doesn't align perfectly in your editor, try viewing it directly on GitHub.
...
Configurations (batch size, learning rate, paths, etc.) can be modified in `utils/config.py`. The training process is divided into clear stages with subsections below for better readability.
...
For questions about the code, open an issue or email: your_email@example.com
